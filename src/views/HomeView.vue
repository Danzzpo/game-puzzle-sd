...
<template>
  <div class="home-container">
    ...
    <nav class="navbar">
      ...
      <div class="nav-right">
        <div v-if="!currentUser" class="auth-buttons">
          ...
          <button class="btn-register" @click="goToLogin">Daftar</button>
        </div>
        ...
        <button class="theme-btn" @click="toggleTheme" title="Ganti Mode">
    ...
    <main class="main-content">
      ...
      <div v-if="!isPlaying" class="content-wrapper">
        <div class="hero-section">
          <div class="hero-text">
            ...
            <div class="cta-buttons">
              <button @click="isPlaying = true" class="btn-primary">▶ Mulai Main</button>
              <button class="btn-secondary">Lihat Koleksi</button>
            </div>
          </div>
          <div class="hero-image">
             <div class="puzzle-card-preview">
               <img src="https://picsum.photos/id/237/500/400" alt="Puzzle Preview">
               <button @click="isPlaying = true" class="solve-btn">🧩 Susun Puzzle</button>
             </div>
          </div>
        </div>
        ...
        <section class="how-it-works">
          ...
          <div class="steps-grid">
            <div class="step-card"><div class="step-circle purple">1</div><h3>Pilih Gambar</h3><p>Pilih gambar dari galeri atau gunakan AI.</p></div>
            <div class="step-card"><div class="step-circle orange">2</div><h3>Atur Kesulitan</h3><p>Sesuaikan jumlah kepingan puzzle.</p></div>
            <div class="step-card"><div class="step-circle green">3</div><h3>Mulai Menyusun</h3><p>Mainkan dengan fitur simpan otomatis.</p></div>
            <div class="step-card"><div class="step-circle pink">4</div><h3>Bagikan</h3><p>Tantang temanmu untuk menyelesaikannya.</p></div>
          </div>
        </section>
      ...
      <div v-else class="game-view">
        <button @click="isPlaying = false" class="btn-back">⬅ Kembali ke Menu</button>
        <GamePuzzle />
      </div>
...
<style scoped>
/* --- CONFIG VARIABLE WARNA (SANGAT PENTING) --- */
:global(.dark-theme) {
  --bg-color: #130f1e;
  --nav-bg: #1e1b2e;
  --text-color: #ffffff;          /* Teks Putih di Dark Mode */
  --text-secondary: #a0a0b0;      /* Teks Abu Terang di Dark Mode */
  --border-color: #333;
...
:global(.light-theme) {
  --bg-color: #f8f9fa;            /* Background Putih Abu di Light Mode */
  --nav-bg: #ffffff;              /* Navbar Putih Bersih */
  --text-color: #1a1a1a;          /* Teks Hitam Pekat di Light Mode */
  --text-secondary: #555555;      /* Teks Abu Gelap di Light Mode */
  --border-color: #e2e8f0;
...
.nav-item {
  ...
  color: var(--text-secondary); /* Gunakan Variable */
  font-weight: 600; font-size: 18px;
  display: flex; align-items: center; gap: 8px; transition: color 0.2s;
}
.nav-item:hover { color: var(--text-color); } /* Hover jadi warna utama */
...
.theme-btn { background: transparent; border: none; cursor: pointer; padding: 5px; width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; }
.theme-icon { font-size: 30px; transition: transform 0.4s ease; display: inline-block; line-height: 1; }

/* HERO */
.hero-section { display: flex; align-items: center; justify-content: space-between; padding: 60px 80px; }
...
/* Paragraf Hero menggunakan Variable Secondary */
...
/* HOW IT WORKS (Teks dipaksa Putih karena Background Ungu Gelap) */
...
/* Override Variable untuk section ini */
...
.feature-card {
  background: var(--card-bg); /* Berubah Putih di Light Mode */
  border: 1px solid var(--border-color);
  border-radius: 12px; padding: 30px; text-align: left; display: flex; gap: 20px;
...
.game-view { padding: 40px; text-align: center; }
.btn-back { margin-bottom: 20px; padding: 10px 20px; cursor: pointer; background: #333; color: white; border: none; border-radius: 5px;}
</style>
