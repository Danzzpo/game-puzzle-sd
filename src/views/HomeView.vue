...
<template>
  <div class="home-container">
    ...
    <div v-if="isMobileMenuOpen" class="mobile-dropdown">
      <router-link to="/puzzles" class="mobile-item">🧩 Puzzle</router-link>
      <router-link to="/popular" class="mobile-item">🔥 Populer</router-link>
      <router-link to="/categories" class="mobile-item">📂 Kategori</router-link>
...
.home-container { background-color: var(--bg-color); color: var(--text-color); min-height: 100vh; font-family: 'Poppins', sans-serif; transition: background-color 0.3s ease; overflow-x: hidden; }
...
.nav-left { display: flex; align-items: center; gap: 8px; font-weight: bold; font-size: 20px; }
.logo-icon { font-size: 28px; }
.highlight { color: #d946ef; }

.nav-center { display: flex; gap: 30px; }
.nav-item { text-decoration: none; color: var(--text-secondary); font-weight: 600; font-size: 16px; transition: 0.2s; }
...
.purple { background: linear-gradient(135deg, #8b5cf6, #6d28d9); } .orange { background: linear-gradient(135deg, #f97316, #ea580c); } .green { background: linear-gradient(135deg, #10b981, #059669); } .pink { background: linear-gradient(135deg, #ec4899, #db2777); }
...
.feature-card { background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 12px; padding: 30px; text-align: left; display: flex; gap: 20px; transition: transform 0.2s; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
...
.purple-glow { background: rgba(139, 92, 246, 0.2); color: #a78bfa; } .orange-glow { background: rgba(249, 115, 22, 0.2); color: #fb923c; } .teal-glow { background: rgba(20, 184, 166, 0.2); color: #2dd4bf; }
